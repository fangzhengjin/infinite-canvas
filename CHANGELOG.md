...
+ [修复] 通过 URL 导入 API 凭据时按 Base URL 更新已有渠道或新增独立渠道，不再覆盖渠道列表第一项；缺少 Base URL 或 API Key、Base URL 非法时不修改配置。
+ [新增] Docker 镜像支持通过启动环境变量同时使用根路径和一个子路径挂载前端。
...
+ [新增] 支持通过跳转参数自动填入 Base URL 和 API Key 配置。
